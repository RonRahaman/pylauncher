...
class CountedCommandGenerator():
    ...
    def next(self):
        ...
        thecount = self.__commandcount__;
        self.__commandcount__ += 1
...
class testGeneratorStuff():
    def testGeneratorListAdd(self):
        ...
        for g in gen:
            count += 1;
            gen.list.append(5)  # this should be in an inherited class
        print("seen %d commands" % count, flush=True)
...
class testCommandlineGeneratorStuff():
    def testFileCommandLineGenerator(self):
        ...
        c = rc["cores"]
        print(r, flush=True);
        assert (r == "b" and str(c) == "1")
        rc = generator.next();
        r = rc["command"];
        c = rc["cores"]
        print(r, flush=True);
        assert (r == "c" and str(c) == "1")
        rc = generator.next();
        r = rc["command"];
        c = rc["cores"]
        print(r, flush=True);
        assert (r == "d" and str(c) == "1")
        try:
            rc = generator.next()
            assert (False)
        except:
...
def ClusterHasSharedFileSystem():
    ...
    return ClusterName() in ["ls4", "ls5", "ls6", "maverick", "stampede", "stampede2", "stampede2-knl", "stampede2-skx",
                             "mic"]
...
def testExecutorTmpDirRandom():
    wd = "a%d" % RandomID();
    print("testing with executor tmpdir", wd, flush=True)
    # cleanup from failed tests
    tmpdir = os.getcwd() + "/" + wd
    if os.path.isdir(tmpdir):
        shutil.rmtree(tmpdir)
...
class testPermanentSSHconnection():
            ...
            if ClusterHasSharedFileSystem():
                try:
                    with open(fn) as outfile:
                        for line in outfile:
                            line = line.strip();
                            print("<<%s>> <<%s>>" % (line, hosts[1]), flush=True)
                            assert (line == hosts[1])
                            break
                except:
                    assert (False)
                ...
                hostread = p.communicate()[0].strip();
                print(hostread, flush=True)
    ...
    def testSSHLeaveStampLoop(self):
        ...
        if ntasks < len(taccpool):  # just to make sure we can run this
            ...
            print("this took %e seconds" % interval, flush=True)
            assert (interval < ntasks * .5)
            time.sleep(nsleep + 1)
            dir = t.completion.stampdir;
...
class testLeaveSSHOutput():
    ...
    def testLeaveModResultsWithTest(self):
        ...
        for d in self.dirs[1:]:
            content = os.listdir(d)
            assert (len(content) >= int(ntasks / len(self.taccpool)))
        content0 = os.listdir(self.dirs[0]);
        print(sorted(content0), flush=True)
...
class MPIExecutor(Executor):
    ...
    def execute(self, command, **kwargs):
        ...
        full_commandline = "mpirun -np {0} {1} {2} {3} ".format(np, self.hfswitch, os.path.join(self.workdir,
                                                                                                hostfilename + str(
                                                                                                    hostfilenumber)),
                                                                self.wrap(command))
...
""")
    ...
    def testIbrunNodeOccupy(self):
        ...
        no_nodes = pool.request_nodes(1);
        print(no_nodes, flush=True)
...
class TestSSHLauncherJobs():
    ...
    def testMulticoreTaskGenerator(self):
        ...
        ntasks = int(self.ncommand / ncores);
        delay = .2
...
def testChDir():
    tmpdir = "pylauncher_tmpdir_test%d" % RandomID();
    tmpfile = RandomFile()
    abs_tmpdir = os.getcwd() + "/" + tmpdir;
    print("working dir:", abs_tmpdir, flush=True)
    ...
    dircontent = os.listdir("..");
    print(".. :", sorted(dircontent), flush=True)
    dircontent = os.listdir(abs_tmpdir);
    print(abs_tmpdir, ":", sorted(dircontent), flush=True)
    assert (tmpfile in dircontent)
    os.system("rm -rf %s" % abs_tmpdir)
